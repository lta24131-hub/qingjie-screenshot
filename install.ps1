...
if (Test-Path -LiteralPath (Join-Path $destination 'QingJie.exe')) {
    ...
    foreach ($name in $files) { $existing = Join-Path $destination $name; if (Test-Path -LiteralPath $existing) { $backupFile=Join-Path $backup $name; New-Item -ItemType Directory -Path (Split-Path $backupFile) -Force | Out-Null; Copy-Item -LiteralPath $existing -Destination $backupFile } }
}
